...
{
    if (v != NULL)
    { // if some memory has been allocated.
        free(v->data);
        free(v);
    }
}
...
void vector_set(vector_t *v, size_t loc, int value)
{
    ...
    if (loc >= v->size)
    {
        ...
        if (point)
        {                           // account for location being greater than size allocated.
            memset(point, 0, size); // bzero copies n bytes with value 0 into point
            size_t x = 0;          
            while (x < v->size)
            {
                point[x] = v->data[x];

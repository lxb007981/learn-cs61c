...
#pragma omp parallel
	{
		...
		for (int i = 0; i < ARRAY_SIZE; i++)
			if (omp_get_thread_num() == i % num_of_threads)
				z[i] = x[i] + y[i];
	}
}
...
#pragma omp parallel
	{
		...
		for (int i = 0; i < ARRAY_SIZE; i++)
			if (thread_id * chunkSize <= i && (thread_id + 1) * chunkSize > i)
				z[i] = x[i] + y[i];
	}
	for (int i = num_of_threads * chunkSize; i < ARRAY_SIZE; i++)
		z[i] = x[i] + y[i];
}

void v_add_native(double* x, double* y, double* z) {
...
int main()
{
	...
	for (int i = 1; i <= num_threads; i++)
	{
		...
		for (int j = 0; j < REPEAT; j++)
			v_add_native(x, y, z);
		run_time = omp_get_wtime() - start_time;
		printf("Native: %d thread(s) took %f seconds\n", i, run_time);
	}
	return 0;
}
